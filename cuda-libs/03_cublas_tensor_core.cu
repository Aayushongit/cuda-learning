// cuBLAS with Tensor Cores: Mixed precision (FP16) for ultra-fast AI training
// Tensor Cores provide up to 8x performance boost for deep learning workloads
...
int main() {
    // Check for Tensor Core capability (compute capability >= 7.0)
    cudaDeviceProp prop;
    ...
    if (prop.major < 7) {
        ...
        printf("Running in compatibility mode...\n\n");
    }
    ...
    // FP16 input, FP32 output using Tensor Cores
    CHECK_CUBLAS(cublasGemmEx(handle,
                             CUBLAS_OP_N, CUBLAS_OP_N,
                             N, M, K,
                             &alpha,
